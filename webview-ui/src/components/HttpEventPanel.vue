...
<template>
  ...
  <section v-else class="HttpEventPanel">
    <div class="tab-container-actions">
      <VsCodeRadioGroupVue v-model="displayReqSwitch" :items="displayReqItems"
        @change="($e: Event) => handleOnChange('andOrSwitch', $e)" class="switch" />

      <button :disabled="noRequest" @click="createRequestScript" title="Create request code in a new Notebook">
        <fa icon="plus" />
      </button>
      <button @click="
        writeToClipboard({
      ...
      <SecondarySelectionAction :items="moreDetailItems" title="..." @onSelect="selectedMoreOptions" />
      <button :disabled="!saveButtonEnabled" @click="downloadBase64File()" title="Save as a file">
        <fa icon="save" />
      </button>
      <button v-if="displayReqSwitch === 'res'" :disabled="!openInEditorEnabled" @click="openInEditor()"
    ...
    <section class="contents" :style="{ width: `${splitterWidth}px` }">
      <div v-if="axiosEvent" class="spPaneWrapper">
        <HttpRequestViewer v-if="displayReqSwitch === 'req'" :res="axiosEvent"
          :header-code-block="codeBlocks.req?.headers ?? ''" :param-code-block="codeBlocks.req?.params ?? ''"
          :contents-code-block="codeBlocks.req?.contents ?? ''"
          :preview-content-type-info="codeBlocks.res.previewContentTypeInfo" :width="splitterWidth"
          :height="splitterHeight" />
        <HttpResponseViewer v-if="displayReqSwitch === 'res'" :res="axiosEvent"
          :header-code-block="codeBlocks.res.headers ?? ''" :contents-code-block="codeBlocks.res.contents ?? ''"
          :preview-content-type-info="codeBlocks.res.previewContentTypeInfo" :url="axiosEvent.entry.request?.url"
          :width="splitterWidth" :height="splitterHeight" />
